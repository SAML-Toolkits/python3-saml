...
class Saml2_Settings(object):
    ...
    def __init__(self, settings=None, custom_base_path=None, sp_validation_only=False):
        ...
        if settings is None:
            ...
            raise Saml2_Error(
                "Unsupported settings object", Saml2_Error.UNSUPPORTED_SETTINGS_OBJECT
            )
    ...
    def _load_settings_from_file(self):
        ...
        if not exists(filename):
            raise Saml2_Error(
                ...
                Saml2_Error.SETTINGS_FILE_NOT_FOUND,
                filename,
            )
    ...
    def _add_default_values(self):
        ...
        self._sp["assertionConsumerService"].setdefault(
            "binding", Saml2_Constants.BINDING_HTTP_POST
        )

        self._sp.setdefault("attributeConsumingService", {})

        self._sp.setdefault("singleLogoutService", {})
        self._sp["singleLogoutService"].setdefault("binding", Saml2_Constants.BINDING_HTTP_REDIRECT)

        self._idp.setdefault("singleLogoutService", {})

        # Related to nameID
        self._sp.setdefault("NameIDFormat", Saml2_Constants.NAMEID_UNSPECIFIED)
        ...
        # Signature Algorithm
        self._security.setdefault("signatureAlgorithm", Saml2_Constants.RSA_SHA256)

        # Digest Algorithm
        self._security.setdefault("digestAlgorithm", Saml2_Constants.SHA256)
            ...
            if not settings.get("idp"):
                ...
                if "security" in settings:
                    ...
                    exists_multix509sign = (
                        "x509certMulti" in idp
                        and "signing" in idp["x509certMulti"]
                        and idp["x509certMulti"]["signing"]
                    )
                    exists_multix509enc = (
                        "x509certMulti" in idp
                        and "encryption" in idp["x509certMulti"]
        ...
        if not isinstance(settings, dict) or not settings:
            ...
            if not settings.get("sp"):
                ...
                if sp.get("attributeConsumingService"):
                    ...
                    if "requestedAttributes" not in attributeConsumingService:
                        ...
                        for req_attrib in attributeConsumingService["requestedAttributes"]:
                            ...
                            if (
                                "attributeValue" in req_attrib
                                and type(req_attrib["attributeValue"]) != list
                            ):
                                errors.append(
                                    "sp_attributeConsumingService_requestedAttributes_attributeValue_type_invalid"
                                )
                            if (
                                "isRequired" in req_attrib
                                and type(req_attrib["isRequired"]) != bool
                            ):
                                errors.append(
                ...
                if "signMetadata" in security and isinstance(security["signMetadata"], dict):
                    if (
                        "keyFileName" not in security["signMetadata"]
                        or "certFileName" not in security["signMetadata"]
                    ):
    ...
    def get_sp_metadata(self):
        ...
        metadata = self.metadata_class.builder(
            self._sp,
            self._security["authnRequestsSigned"],
        ...
        add_encryption = (
            self._security["wantNameIdEncrypted"] or self._security["wantAssertionsEncrypted"]
        )
        ...
        if "signMetadata" in self._security and self._security["signMetadata"] is not False:
            if self._security["signMetadata"] is True:
                ...
                if not cert:
                    raise Saml2_Error(
                        ...
                        Saml2_Error.PUBLIC_CERT_FILE_NOT_FOUND,
                    )
                cert_metadata = cert
            ...
            metadata = self.metadata_class.sign_metadata(
                metadata, key_metadata, cert_metadata, signature_algorithm, digest_algorithm
            )
    ...
    def validate_metadata(self, xml):
        ...
        errors = []
        root = Saml2_XML.validate_xml(xml, "saml-schema-metadata-2.0.xsd", self._debug)
        if isinstance(root, str):
            errors.append(root)
        else:
            if root.tag != "{%s}EntityDescriptor" % Saml2_Constants.NS_MD:
                errors.append("noEntityDescriptor_xml")
            else:
                if (
                    len(root.findall(".//md:SPSSODescriptor", namespaces=Saml2_Constants.NSMAP))
                ) != 1:
    ...
    def format_idp_cert(self):
        ...
        """
        self._idp["x509cert"] = Saml2_Utils.format_cert(self._idp["x509cert"])
        ...
        if "x509certMulti" in self._idp:
            if "signing" in self._idp["x509certMulti"]:
                for idx in range(len(self._idp["x509certMulti"]["signing"])):
                    self._idp["x509certMulti"]["signing"][idx] = Saml2_Utils.format_cert(
                        self._idp["x509certMulti"]["signing"][idx]
                    )

            if "encryption" in self._idp["x509certMulti"]:
                for idx in range(len(self._idp["x509certMulti"]["encryption"])):
                    self._idp["x509certMulti"]["encryption"][idx] = Saml2_Utils.format_cert(
                        self._idp["x509certMulti"]["encryption"][idx]
                    )
    ...
    def format_sp_cert(self):
        ...
        Formats the SP cert.
        """
        self._sp["x509cert"] = Saml2_Utils.format_cert(self._sp["x509cert"])

    def format_sp_cert_new(self):
        """
        Formats the SP cert.
        """
        self._sp["x509certNew"] = Saml2_Utils.format_cert(self._sp["x509certNew"])

    def format_sp_key(self):
        """
        Formats the private key.
        """
        self._sp["privateKey"] = Saml2_Utils.format_private_key(self._sp["privateKey"])
