...
from saml2.xmlparser import fromstring
...
class Saml2_Utils_Test(unittest.TestCase):
    ...
    def assertRaisesRegex(self, exception, regexp, msg=None):
        if hasattr(unittest.TestCase, "assertRaisesRegex"):
            return super(Saml2_Utils_Test, self).assertRaisesRegex(exception, regexp, msg=msg)
        else:
    ...
    def testFormatCert(self):
        """
        Tests the format_cert method of the Saml2_Utils
        """
        ...
        formated_cert1 = Saml2_Utils.format_cert(cert)
        self.assertIn("-----BEGIN CERTIFICATE-----", formated_cert1)
        self.assertIn("-----END CERTIFICATE-----", formated_cert1)

        formated_cert2 = Saml2_Utils.format_cert(cert, True)
        self.assertEqual(formated_cert1, formated_cert2)

        formated_cert3 = Saml2_Utils.format_cert(cert, False)
    ...
    def testFormatPrivateKey(self):
        """
        Tests the format_private_key method of the Saml2_Utils
        """
        ...
        formated_key = Saml2_Utils.format_private_key(key, True)
        ...
        formated_key = Saml2_Utils.format_private_key(key, False)
        ...
        formated_key_2 = Saml2_Utils.format_private_key(key_2, True)
        ...
        formated_key_2 = Saml2_Utils.format_private_key(key_2, False)
        ...
        formated_key_3 = Saml2_Utils.format_private_key(key_3, True)
        ...
        formated_key_3 = Saml2_Utils.format_private_key(key_3, False)
    ...
    def testRedirect(self):
        """
        Tests the redirect method of the Saml2_Utils
        """
        request_data = {"http_host": "example.com"}

        # Check relative and absolute
        hostname = Saml2_Utils.get_self_host(request_data)
        url = "http://%s/example" % hostname
        url2 = "/example"

        target_url = Saml2_Utils.redirect(url, {}, request_data)
        target_url2 = Saml2_Utils.redirect(url2, {}, request_data)
        ...
        target_url3 = Saml2_Utils.redirect(url3, {}, request_data)
        self.assertIn("test=true", target_url3)
        with self.assertRaises(Exception) as context:
            Saml2_Utils.redirect(url4, {}, request_data)
        ...
        target_url5 = Saml2_Utils.redirect(url, parameters1, request_data)
        self.assertEqual("http://%s/example?value1=a" % hostname, target_url5)

        target_url6 = Saml2_Utils.redirect(url3, parameters1, request_data)
        ...
        target_url7 = Saml2_Utils.redirect(url, parameters2, request_data)
        parameters2_decoded = {
        ...
        parameters3_str = "&".join(
            (parameters3_decoded[x] for x in parameters3.keys() if x in parameters3_decoded)
        )
        target_url8 = Saml2_Utils.redirect(url, parameters3, request_data)
        self.assertEqual("http://%s/example?%s" % (hostname, parameters3_str), target_url8)

    def testGetselfhost(self):
        """
        Tests the get_self_host method of the Saml2_Utils
        """
        request_data = {}
        with self.assertRaises(Exception) as context:
            Saml2_Utils.get_self_url_host(request_data)
            exception = context.exception
            self.assertIn("No hostname defined", str(exception))

        request_data = {"server_name": "example.com"}
        self.assertEqual("example.com", Saml2_Utils.get_self_host(request_data))

        request_data = {"http_host": "example.com"}
        self.assertEqual("example.com", Saml2_Utils.get_self_host(request_data))

        request_data = {"http_host": "example.com:443"}
        self.assertEqual("example.com:443", Saml2_Utils.get_self_host(request_data))

        request_data = {"http_host": "example.com:ok"}
        self.assertEqual("example.com:ok", Saml2_Utils.get_self_host(request_data))

    def testisHTTPS(self):
        """
        Tests the is_https method of the Saml2_Utils
        """
        request_data = {"https": "off"}
        self.assertFalse(Saml2_Utils.is_https(request_data))

        request_data = {"https": "on"}
        self.assertTrue(Saml2_Utils.is_https(request_data))
    ...
    def testGetSelfURLhost(self):
        """
        Tests the get_self_url_host method of the Saml2_Utils
        """
        request_data = {"http_host": "example.com"}
        self.assertEqual("http://example.com", Saml2_Utils.get_self_url_host(request_data))

        request_data["server_port"] = "80"
        self.assertEqual("http://example.com", Saml2_Utils.get_self_url_host(request_data))

        request_data["server_port"] = "81"
        self.assertEqual("http://example.com:81", Saml2_Utils.get_self_url_host(request_data))

        request_data["server_port"] = "443"
        self.assertEqual("https://example.com", Saml2_Utils.get_self_url_host(request_data))
        ...
        self.assertEqual("https://example.com:444", Saml2_Utils.get_self_url_host(request_data))
        ...
        with self.assertRaises(Exception) as context:
            Saml2_Utils.get_self_url_host(request_data2)
            exception = context.exception
            self.assertIn("No hostname defined", str(exception))

    def testGetSelfURL(self):
        """
        Tests the get_self_url method of the Saml2_Utils
        """
        request_data = {"http_host": "example.com"}
        url = Saml2_Utils.get_self_url_host(request_data)
        self.assertEqual(url, Saml2_Utils.get_self_url(request_data))
        ...
        self.assertEqual(url + "index.html", Saml2_Utils.get_self_url(request_data))
        ...
        self.assertEqual(url + "/index.html?testing", Saml2_Utils.get_self_url(request_data))

        request_data["request_uri"] = "/test/index.html?testing"
        self.assertEqual(url + "/test/index.html?testing", Saml2_Utils.get_self_url(request_data))

        request_data["request_uri"] = "https://example.com/testing"
        self.assertEqual(url + "/testing", Saml2_Utils.get_self_url(request_data))

    def testGetSelfURLNoQuery(self):
        """
        Tests the get_self_url_no_query method of the Saml2_Utils
        """
        request_data = {"http_host": "example.com", "script_name": "/index.html"}
        url = Saml2_Utils.get_self_url_host(request_data) + request_data["script_name"]
        self.assertEqual(url, Saml2_Utils.get_self_url_no_query(request_data))

        request_data["path_info"] = "/test"
        self.assertEqual(url + "/test", Saml2_Utils.get_self_url_no_query(request_data))

    def testGetSelfRoutedURLNoQuery(self):
        """
        Tests the get_self_routed_url_no_query method of the Saml2_Utils
        """
        request_data = {
        ...
        url = Saml2_Utils.get_self_url_host(request_data) + "/example1/route"
        self.assertEqual(url, Saml2_Utils.get_self_routed_url_no_query(request_data))

        request_data_2 = {
            "http_host": "example.com",
            "request_uri": "",
        }
        url_2 = Saml2_Utils.get_self_url_host(request_data_2)
        self.assertEqual(url_2, Saml2_Utils.get_self_routed_url_no_query(request_data_2))

        request_data_3 = {
            "http_host": "example.com",
        }
        url_3 = Saml2_Utils.get_self_url_host(request_data_3)
        self.assertEqual(url_3, Saml2_Utils.get_self_routed_url_no_query(request_data_3))
        ...
        url_4 = Saml2_Utils.get_self_url_host(request_data_4) + "/example1/route/test/"
        self.assertEqual(url_4, Saml2_Utils.get_self_routed_url_no_query(request_data_4))
        ...
        url_5 = Saml2_Utils.get_self_url_host(request_data_5) + "/example1/route/test/"
        self.assertEqual(url_5, Saml2_Utils.get_self_routed_url_no_query(request_data_5))

        request_data_6 = {
            "http_host": "example.com",
            "request_uri": "/example1/route/test/",
        }
        url_6 = Saml2_Utils.get_self_url_host(request_data_6) + "/example1/route/test/"
        self.assertEqual(url_6, Saml2_Utils.get_self_routed_url_no_query(request_data_6))
    ...
    def testGetStatus(self):
        ...
        status = Saml2_Utils.get_status(dom)
        ...
        status2 = Saml2_Utils.get_status(dom2)
        self.assertEqual(Saml2_Constants.STATUS_RESPONDER, status2["code"])
        ...
        status3 = Saml2_Utils.get_status(dom3)
        self.assertEqual(Saml2_Constants.STATUS_RESPONDER, status3["code"])
        ...
        with self.assertRaisesRegex(Exception, "Missing Status Code on response"):
            Saml2_Utils.get_status(dom_inv2)

    def testParseDuration(self):
        """
        Tests the parse_duration method of the Saml2_Utils
        """
        duration = "PT1393462294S"
        timestamp = 1393876825

        parsed_duration = Saml2_Utils.parse_duration(duration, timestamp)
        self.assertEqual(2787339119, parsed_duration)

        parsed_duration_2 = Saml2_Utils.parse_duration(duration)
        self.assertTrue(parsed_duration_2 > parsed_duration)

        invalid_duration = "PT1Y"
        with self.assertRaises(Exception) as context:
            Saml2_Utils.parse_duration(invalid_duration)
            exception = context.exception
            self.assertIn("Unrecognised ISO 8601 date format", str(exception))

        new_duration = "P1Y1M"
        parsed_duration_4 = Saml2_Utils.parse_duration(new_duration, timestamp)
        self.assertEqual(1428091225, parsed_duration_4)

        neg_duration = "-P14M"
        parsed_duration_5 = Saml2_Utils.parse_duration(neg_duration, timestamp)
        self.assertEqual(1357243225, parsed_duration_5)

    def testParseSAML2Time(self):
        """
        Tests the parse_SAML_to_time method of the Saml2_Utils
        """
        time = 1386650371
        saml_time = "2013-12-10T04:39:31Z"
        self.assertEqual(time, Saml2_Utils.parse_SAML_to_time(saml_time))
        with self.assertRaises(Exception) as context:
            Saml2_Utils.parse_SAML_to_time("invalidSAMLTime")
            exception = context.exception
        ...
        self.assertEqual(time, Saml2_Utils.parse_SAML_to_time(saml_time2))

        # Now test if toolkit supports microseconds
        saml_time3 = "2013-12-10T04:39:31.120240Z"
        self.assertEqual(time, Saml2_Utils.parse_SAML_to_time(saml_time3))

        # Now test if toolkit supports nanoseconds
        saml_time4 = "2013-12-10T04:39:31.120240360Z"
        self.assertEqual(time, Saml2_Utils.parse_SAML_to_time(saml_time4))

    def testParseTime2SAML(self):
        """
        Tests the parse_time_to_SAML method of the Saml2_Utils
        """
        time = 1386650371
        saml_time = "2013-12-10T04:39:31Z"
        self.assertEqual(saml_time, Saml2_Utils.parse_time_to_SAML(time))
        with self.assertRaises(Exception) as context:
            Saml2_Utils.parse_time_to_SAML("invalidtime")
            exception = context.exception
            self.assertIn("could not convert string to float", str(exception))

    def testGetExpireTime(self):
        """
        Tests the get_expire_time method of the Saml2_Utils
        """
        self.assertEqual(None, Saml2_Utils.get_expire_time())
        self.assertNotEqual(None, Saml2_Utils.get_expire_time("PT360000S"))

        self.assertEqual(
            "1291955971", Saml2_Utils.get_expire_time("PT360000S", "2010-12-10T04:39:31Z")
        )
        self.assertEqual("1291955971", Saml2_Utils.get_expire_time("PT360000S", 1291955971))

        self.assertNotEqual(
            "3311642371", Saml2_Utils.get_expire_time("PT360000S", "2074-12-10T04:39:31Z")
        )
        self.assertNotEqual("3311642371", Saml2_Utils.get_expire_time("PT360000S", 1418186371))
    ...
    def _generate_name_id_element(self, name_qualifier):
        ...
        raw_name_id = Saml2_Utils.generate_name_id(
            name_id_value,
            entity_id,
            name_id_format,
            nq=name_qualifier,
    ...
    def testNameidGenerationIncludesNameQualifierAttribute(self):
        """
        Tests the inclusion of NameQualifier in the generateNameId method of the Saml2_Utils
        """
    ...
    def testNameidGenerationDoesNotIncludeNameQualifierAttribute(self):
        """
        Tests the (not) inclusion of NameQualifier in the generateNameId method of the Saml2_Utils
        """
    ...
    def testGenerateNameIdWithoutFormat(self):
        """
        Tests the generateNameId method of the Saml2_Utils
        """
        name_id_value = "ce998811003f4e60f8b07a311dc641621379cfde"
        name_id_format = None

        name_id = Saml2_Utils.generate_name_id(name_id_value, None, name_id_format)
        expected_name_id = "<saml:NameID>ce998811003f4e60f8b07a311dc641621379cfde</saml:NameID>"
    ...
    def testGenerateNameIdWithSPNameQualifier(self):
        ...
        name_id = Saml2_Utils.generate_name_id(name_id_value, entity_id, name_id_format)
        expected_name_id = '<saml:NameID SPNameQualifier="http://stuff.com/endpoints/metadata.php" Format="urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified">ce998811003f4e60f8b07a311dc641621379cfde</saml:NameID>'
        self.assertEqual(expected_name_id, name_id)

        settings_info = self.loadSettingsJSON()
        x509cert = settings_info["idp"]["x509cert"]
        key = Saml2_Utils.format_cert(x509cert)

        name_id_enc = Saml2_Utils.generate_name_id(name_id_value, entity_id, name_id_format, key)
    ...
    def testGenerateNameIdWithoutSPNameQualifier(self):
        ...
        name_id = Saml2_Utils.generate_name_id(name_id_value, None, name_id_format)
        expected_name_id = '<saml:NameID Format="urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified">ce998811003f4e60f8b07a311dc641621379cfde</saml:NameID>'
        self.assertEqual(expected_name_id, name_id)
        ...
        name_id_enc = Saml2_Utils.generate_name_id(name_id_value, None, name_id_format, key)
        expected_name_id_enc = '<saml:EncryptedID><xenc:EncryptedData xmlns:xenc="http://www.w3.org/2001/04/xmlenc#" xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" Type="http://www.w3.org/2001/04/xmlenc#Element">\n<xenc:EncryptionMethod Algorithm="http://www.w3.org/2001/04/xmlenc#aes128-cbc"/>\n<dsig:KeyInfo xmlns:dsig="http://www.w3.org/2000/09/xmldsig#">\n<xenc:EncryptedKey>\n<xenc:EncryptionMethod Algorithm="http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p"/>\n<xenc:CipherData>\n<xenc:CipherValue>'
        self.assertIn(expected_name_id_enc, name_id_enc)

    def testCalculateX509Fingerprint(self):
        """
        Tests the calculateX509Fingerprint method of the Saml2_Utils
        """
        ...
        self.assertEqual(None, Saml2_Utils.calculate_x509_fingerprint(key))
        self.assertEqual(
            "afe71c28ef740bc87425be13a2263d37971da1f9",
            Saml2_Utils.calculate_x509_fingerprint(cert),
        )
        self.assertEqual(
            "afe71c28ef740bc87425be13a2263d37971da1f9",
            Saml2_Utils.calculate_x509_fingerprint(cert, "sha1"),
        )

        self.assertEqual(
            "c51cfa06c7a49767f6eab18238eae1c56708e29264da3d11f538a12cd2c357ba",
            Saml2_Utils.calculate_x509_fingerprint(cert, "sha256"),
        )

        self.assertEqual(
            "bc5826e6f9429247254bae5e3c650e6968a36a62d23075eb168134978d88600559c10830c28711b2c29c7947c0c2eb1d",
            Saml2_Utils.calculate_x509_fingerprint(cert, "sha384"),
        )
        ...
        self.assertEqual(
            ...
            Saml2_Utils.calculate_x509_fingerprint(cert, "sha512"),
        )

    def testDeleteLocalSession(self):
        """
        Tests the delete_local_session method of the Saml2_Utils
        """
        global local_session_test
        local_session_test = 1

        Saml2_Utils.delete_local_session()
        self.assertEqual(1, local_session_test)

        dscb = lambda: self.session_cear()
        Saml2_Utils.delete_local_session(dscb)
        self.assertEqual(0, local_session_test)

    def session_cear(self):
        """
        Auxiliar method to test the delete_local_session method of the Saml2_Utils
        """
        global local_session_test
    ...
    def testFormatFingerPrint(self):
        """
        Tests the format_finger_print method of the Saml2_Utils
        """
        finger_print_1 = "AF:E7:1C:28:EF:74:0B:C8:74:25:BE:13:A2:26:3D:37:97:1D:A1:F9"
        self.assertEqual(
            "afe71c28ef740bc87425be13a2263d37971da1f9",
            Saml2_Utils.format_finger_print(finger_print_1),
        )

        finger_print_2 = "afe71c28ef740bc87425be13a2263d37971da1f9"
        self.assertEqual(
            "afe71c28ef740bc87425be13a2263d37971da1f9",
            Saml2_Utils.format_finger_print(finger_print_2),
        )

    def testDecryptElement(self):
        """
        Tests the decrypt_element method of the Saml2_Utils
        """
        settings = Saml2_Settings(self.loadSettingsJSON())
        ...
        encrypted_nameid_nodes = dom_nameid_enc.find(
            ".//saml:EncryptedID", namespaces=Saml2_Constants.NSMAP
        )
        encrypted_data = encrypted_nameid_nodes[0]
        decrypted_nameid = Saml2_Utils.decrypt_element(encrypted_data, key)
        ...
        encrypted_assertion_enc_nodes = dom_assertion_enc.find(
            ".//saml:EncryptedAssertion", namespaces=Saml2_Constants.NSMAP
        )
        encrypted_data_assert = encrypted_assertion_enc_nodes[0]

        decrypted_assertion = Saml2_Utils.decrypt_element(encrypted_data_assert, key)
        self.assertEqual("{%s}Assertion" % Saml2_Constants.NS_SAML, decrypted_assertion.tag)
        self.assertEqual(
            "_6fe189b1c241827773902f2b1d3a843418206a5c97", decrypted_assertion.get("ID")
        )

        encrypted_nameid_nodes = decrypted_assertion.xpath(
            "./saml:Subject/saml:EncryptedID", namespaces=Saml2_Constants.NSMAP
        )
        ...
        decrypted_nameid = Saml2_Utils.decrypt_element(encrypted_data, key2)
        ...
        decrypted_nameid = Saml2_Utils.decrypt_element(encrypted_data, key3)
        self.assertIn("{%s}NameID" % (Saml2_Constants.NS_SAML), decrypted_nameid.tag)
        self.assertEqual("457bdb600de717891c77647b0806ce59c089d5b8", decrypted_nameid.text)
        ...
        with self.assertRaisesRegex(Exception, "(1, 'failed to decrypt')"):
            Saml2_Utils.decrypt_element(encrypted_data, key4)
        ...
        with self.assertRaisesRegex(Exception, "(1, 'failed to decrypt')"):
            Saml2_Utils.decrypt_element(encrypted_data_2, key)
        ...
        with self.assertRaisesRegex(Exception, "(1, 'failed to decrypt')"):
            Saml2_Utils.decrypt_element(encrypted_data_3, key)

    def testDecryptElementInplace(self):
        """
        Tests the decrypt_element method of the Saml2_Utils with inplace=True
        """
        settings = Saml2_Settings(self.loadSettingsJSON())
        ...
        encrypted_node = dom.xpath(
            "//saml:EncryptedID/xenc:EncryptedData", namespaces=Saml2_Constants.NSMAP
        )[0]

        # can be decrypted twice when copy the node first
        for _ in range(2):
            decrypted_nameid = Saml2_Utils.decrypt_element(encrypted_node, key, inplace=False)
            self.assertIn("NameID", decrypted_nameid.tag)
            self.assertEqual("2de11defd199f8d5bb63f9b7deb265ba5c675c10", decrypted_nameid.text)

        # can only be decrypted once in place
        decrypted_nameid = Saml2_Utils.decrypt_element(encrypted_node, key, inplace=True)
        self.assertIn("NameID", decrypted_nameid.tag)
        self.assertEqual("2de11defd199f8d5bb63f9b7deb265ba5c675c10", decrypted_nameid.text)

        # can't be decrypted twice since it has been decrypted inplace
        with self.assertRaisesRegex(Exception, "(1, 'failed to decrypt')"):
            Saml2_Utils.decrypt_element(encrypted_node, key, inplace=True)

    def testAddSign(self):
        """
        Tests the add_sign method of the Saml2_Utils
        """
        settings = Saml2_Settings(self.loadSettingsJSON())
        key = settings.get_sp_key()
        ...
        xml_authn_signed_2 = compat.to_string(
            Saml2_Utils.add_sign(xml_authn_dom.toxml(), key, cert)
        )
        ...
        xml_authn_signed_3 = compat.to_string(
            Saml2_Utils.add_sign(xml_authn_dom.firstChild.toxml(), key, cert)
        )
        ...
        xml_authn_signed_4 = compat.to_string(Saml2_Utils.add_sign(xml_authn_etree, key, cert))
        ...
        xml_authn_signed_5 = compat.to_string(Saml2_Utils.add_sign(xml_authn_etree, key, cert))
    ...
    def testAddSignCheckAlg(self):
        ...
        xml_authn_signed_3 = compat.to_string(
            Saml2_Utils.add_sign(
                ...
                cert,
                False,
                Saml2_Constants.RSA_SHA384,
                Saml2_Constants.SHA512,
            )
        )
        self.assertIn("<ds:SignatureValue>", xml_authn_signed_3)
        self.assertIn(
    ...
    def testValidateSign(self):
        """
        Tests the validate_sign method of the Saml2_Utils
        """
        settings = Saml2_Settings(self.loadSettingsJSON())
        ...
        fingerprint_2 = Saml2_Utils.calculate_x509_fingerprint(cert_2)
        fingerprint_2_256 = Saml2_Utils.calculate_x509_fingerprint(cert_2, "sha256")

        try:
            self.assertFalse(Saml2_Utils.validate_sign("", cert))
        ...
        self.assertFalse(
            Saml2_Utils.validate_metadata_sign(xml_metadata_signed, cert, validatecert=True)
        )
        ...
        self.assertTrue(
            Saml2_Utils.validate_metadata_sign(xml_metadata_signed_2, None, fingerprint_2)
        )
        ...
        # expired cert
        self.assertTrue(Saml2_Utils.validate_sign(xml_response_msg_signed, cert))
        # expired cert, verified it
        self.assertFalse(
            Saml2_Utils.validate_sign(xml_response_msg_signed, cert, validatecert=True)
        )
        ...
        self.assertFalse(
            Saml2_Utils.validate_sign(xml_response_msg_signed, cert_x, validatecert=True)
        )
        ...
        self.assertTrue(
            Saml2_Utils.validate_sign(xml_response_msg_signed_2, None, fingerprint_2, "sha1")
        )
        self.assertTrue(
            Saml2_Utils.validate_sign(xml_response_msg_signed_2, None, fingerprint_2_256, "sha256")
        )
        ...
        # expired cert
        self.assertTrue(Saml2_Utils.validate_sign(xml_response_assert_signed, cert))
        # expired cert, verified it
        self.assertFalse(
            Saml2_Utils.validate_sign(xml_response_assert_signed, cert, validatecert=True)
        )
        ...
        self.assertTrue(
            Saml2_Utils.validate_sign(xml_response_assert_signed_2, None, fingerprint_2)
        )
        ...
        # expired cert
        self.assertTrue(Saml2_Utils.validate_sign(xml_response_double_signed, cert))
        # expired cert, verified it
        self.assertFalse(
            Saml2_Utils.validate_sign(xml_response_double_signed, cert, validatecert=True)
        )
        ...
        self.assertTrue(
            Saml2_Utils.validate_sign(xml_response_double_signed_2, None, fingerprint_2)
        )

        dom = parseString(xml_response_msg_signed_2)
        self.assertTrue(Saml2_Utils.validate_sign(dom.toxml(), cert_2))
        ...
        dom.firstChild.getAttributeNode(
            "ID"
        ).nodeValue = "_34fg27g212d63k1f923845324475802ac0fc24530b"
        # Reference validation failed
        self.assertFalse(Saml2_Utils.validate_sign(dom.toxml(), cert_2))

        invalid_fingerprint = "afe71c34ef740bc87434be13a2263d31271da1f9"
        # Wrong fingerprint
        self.assertFalse(
            Saml2_Utils.validate_metadata_sign(xml_metadata_signed_2, None, invalid_fingerprint)
        )

        dom_2 = parseString(xml_response_double_signed_2)
        self.assertTrue(Saml2_Utils.validate_sign(dom_2.toxml(), cert_2))
        dom_2.firstChild.firstChild.firstChild.nodeValue = "https://example.com/other-idp"
        # Modified message
        self.assertFalse(Saml2_Utils.validate_sign(dom_2.toxml(), cert_2))
        ...
        assert_elem_3.setAttributeNS(Saml2_Constants.NS_SAML, "xmlns:saml", Saml2_Constants.NS_SAML)
        self.assertFalse(Saml2_Utils.validate_sign(assert_elem_3.toxml(), cert_2))
    ...
    def testNormalizeUrl(self):
        ...
        self.assertIn(base_url, Saml2_Utils.normalize_url(capital_scheme))
        self.assertIn(base_url, Saml2_Utils.normalize_url(capital_domain))
        self.assertNotIn(base_url, Saml2_Utils.normalize_url(capital_path))
        self.assertNotIn(base_url, Saml2_Utils.normalize_url(capital_all))
